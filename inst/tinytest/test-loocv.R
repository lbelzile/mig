...
lcv <- sapply(1:n, function(i){mig_kdens_arma(x = x[-i,], newdata = x[i, ,drop=FALSE], Omega = Omega, beta = beta, logd = TRUE)})
lcv2 <- as.numeric(mig_loo(x = x,  Omega = Omega, beta = beta))
expect_equal(lcv, lcv2)

lcv <- sapply(1:n, function(i){tnorm_kdens_arma(x = x[-i,], newdata = x[i, ,drop=FALSE], Omega = Omega, beta = beta, logd = TRUE)})
...
lcv <- sapply(1:n, function(i){gauss_kdens_arma(x = tx[-i,], newdata = tx[i, ,drop=FALSE], Sigma = Sigma, logweights = logjac[-i], logd = TRUE)})
